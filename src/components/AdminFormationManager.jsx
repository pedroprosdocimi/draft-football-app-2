...
import FormationSlotEditor from './FormationSlotEditor.jsx';
...
export default function AdminFormationManager() {
  ...
  const handleSlotsSaved = (formationName, savedSlots) => {
    setFormations((current) => current.map((f) => {
      if (f.name !== formationName) return f;
      const updatedSlots = f.slots.map((s) => {
        const saved = savedSlots.find((ss) => ss.position === s.position);
        return saved ? { ...s, x: saved.x, y: saved.y } : s;
      });
      return { ...f, slots: updatedSlots };
    }));
    setMessage(`Posicoes da formacao ${formationName} salvas.`);
  };
        ...
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-400">
              Ative ou desative quais esquemas podem aparecer na escolha do draft. Clique em "Editar" para reposicionar os jogadores no campo.
            </p>
          </div>
            ...
            <table className="w-full min-w-[920px] text-sm">
              <thead>
                <tr className="border-b border-gray-800 text-left text-xs uppercase tracking-wide text-gray-500">
                  ...
                  <th className="pb-3 pr-4 font-medium">Posicoes</th>
                    ...
                    <tr key={formation.name} className="align-top">
                      ...
                      <td className="py-4 pr-4">
                        <button
                          ...
                          className="rounded-lg border border-white/10 px-3 py-1.5 text-xs text-slate-300 hover:border-emerald-500/40 hover:bg-emerald-950/40 hover:text-emerald-300"
                        >
                          Editar posicoes
                        </button>
                      </td>
